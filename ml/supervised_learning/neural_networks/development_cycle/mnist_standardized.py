...
# An MNIST data (pre-shuffled) loader that splits data into training, validation & test sets & standardizes the dataset.
import numpy as np
from ml.util import load_images, load_labels, one_hot_encoding, standardize_sets
...
X_train_raw = load_images(TRAIN_IMAGE)
X_test_raw = load_images(TEST_IMAGE)  # To ensure best practice: np.random.shuffle(X_test_all)
X_train, X_test_all = standardize_sets(X_train_raw, X_test_raw)
